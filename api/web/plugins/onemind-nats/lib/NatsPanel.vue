                ...
                <div class='row g-2'>
                    ...
                    <div class='col-6'>
                        ...
                        <div
                            ...
                            style='font-size: 13px;'
                        >
                            {{ status.subjects?.length || 0 }} subscribed
                        </div>
                    </div>
                </div>
            </div>
...
async function fetchStatus() {
    try {
        ...
        if (res.ok) {
            const data = await res.json() as { connected?: boolean; server?: string; subjects?: string[]; messageCount?: number };
...
function connectWebSocket() {
    ...
    try {
        ...
        ws.onmessage = (event) => {
            try {
                const msg = JSON.parse(event.data as string) as { type?: string; subject?: string; data?: unknown };
                if (msg.type === 'message') {
